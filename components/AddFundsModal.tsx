...
interface AddFundsModalProps {
    ...
    onAddFunds: (amount: number, description: string, date: Date) => void;
}

export function AddFundsModal({ visible, onClose, wallet, onAddFunds }: AddFundsModalProps) {
    ...
    const handleSave = () => {
        ...
        if (!isNaN(value) && value > 0) {
            onAddFunds(value, description.trim() || '잔액 추가', date);
            setAmount('');
            setDescription('');
            setDate(new Date());
            onClose();
        ...
        >
            <View style={styles.centeredView}>
                <View style={styles.modalView}>
                    <Text style={styles.modalTitle}>잔액 추가 ({wallet.currency})</Text>

                    <Text style={styles.label}>추가할 금액</Text>
